...
从已有 Release 复制同一批已校验引擎 ZIP 到目标预发布 Draft。先停止所有正在向
同名资产写入的上传进程，再填写来源 tag、目标 tag 和目标完整提交 SHA。
该流程读取目标不可变提交中的目录，验证全部 15 个来源文件的大小和 SHA-256，
逐个下载并再次校验后上传；不执行归档内程序，不覆盖已完成资产。
身份不匹配、摘要不一致或传输失败都会停止，不改变 tag、正式频道或发布状态。
目标应用仍须从原有固定 tag 构建并通过全部发布验收。
...
四个平台统一使用 `scripts/upload_release_assets.py` 按固定资产清单逐文件上传，单次超时
15 分钟、每个文件最多 4 次尝试。GitHub 返回 HTTP 500 或响应丢失时，先重新读取文件状态：
大小和 SHA-256 一致的完整资产直接复用；只清理同一 Draft 中的失败 `starter` 记录。
完整文件摘要不一致时立即失败，禁止 `--clobber` 覆盖；每次写操作前重新核验 tag、提交和
Draft 身份。上传步骤整体超过 90 分钟也必须失败，不能跳过资产或手工提前公开。

如果失败后需要重新构建会改变字节的成品，应使用新版本号，保留原 tag 与已完成的资产。
...
- 默认模型：`kata1-tf3-b11c768-s12002M-d6304M.bin.gz`
- 默认模型 SHA-256：`4a6312e80faadee7b7dd28689a2e87a1efb4640c10132f16290da7a17b4c6d9e`
- 默认模型大小：`262,017,809` 字节；架构：`transformer`；最低 KataGo：`1.17.0`
...
占位符和六种语言直链表。已人工审核的 `next-2026-08-19.1`、`next-2026-08-19.2`、
`next-2026-08-19.3`、`next-2026-08-19.4` 正文被设为不可覆盖；不要用此生成工作流修改它们。
