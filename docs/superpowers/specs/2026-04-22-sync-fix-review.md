...
上位契约：[docs/SNAPSHOT_NODE_KIND.md](/docs/SNAPSHOT_NODE_KIND.md)
...
- [fox-marker-valid-gate-fix-design.md](/docs/superpowers/specs/2026-04-22-fox-marker-valid-gate-fix-design.md) 第 2 节明确：fox recovery 路径只用 `stones + moveNumber`，不依赖 marker。
- [SNAPSHOT_NODE_KIND.md](/docs/SNAPSHOT_NODE_KIND.md) 第 3.1 节：`marker` 是增强信号，不是硬前提。
...
- [SNAPSHOT_NODE_KIND.md](/docs/SNAPSHOT_NODE_KIND.md)：同一冲突快照的判定基于归一化远端身份，不基于原始 `snapshotCodes` 整帧全等。marker 抖动不能把同一冲突拆成不同 key。
...
- [readboard-early-game-history-design.md](/docs/superpowers/specs/2026-04-22-readboard-early-game-history-design.md) 第 5.1 节明确要求接受"无 marker + 单颗新增 + recoveryMoveNumber == current + 1"的帧进入增量路径。
...
- `GraphPoint` 的 x 坐标由 `graphPointX(moveNumber)` 计算，是对 `moveNumber` 的线性映射。
- points 按历史遍历顺序（root → end）添加，`moveNumber` 单调递增，x 坐标天然单调递增。
- 在 x 单调的序列上，最小正间距一定出现在相邻元素之间，O(n) 相邻比较与 O(n²) 全局比较结果一致。
- `QuickOverviewPoint` 同理，按 moveNumber 顺序构造。
...
| 边界条件 | WinrateGraph 排序假设成立（x 单调递增）。空列表、单元素列表安全（循环不进入） |
