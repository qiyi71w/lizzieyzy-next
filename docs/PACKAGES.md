...
- RTX 40/50 默认推荐 CUDA；TensorRT 是 RTX 30 系及以下 NVIDIA 显卡的可选方案，可在软件内按需安装
- NVIDIA 驱动 `570.65` 及以上直接加载；`528.33–570.64` 首次执行轻量真实推理探测；更旧驱动显示明确修复状态
...
- CUDA / TensorRT 吞吐与功耗对比必须固定模型、配置和缓存，并使用 [RTX 50 性能验收流程](RTX50_PERFORMANCE.md) 记录真实进程、功耗、显存和 KataGo 原始指标
- Release 可附带 TensorRT 可选预装分卷包，但它不是默认推荐下载；必须下载全部 `.7z.00N` 并用 7-Zip 从 `.001` 解压
...
- 当前维护版公开 release 主列表保留 13 个首次下载稳定资产；`windows64.core-update.zip` 是已有免安装用户的日常小更新资产，不是首次下载包。
- TensorRT 分卷包是可选资产，不计入普通用户主推荐路径；只下载 `.7z.001` 没用，必须下载全部 `.7z.00N`。
...
这些包不计入 13 个稳定首选资产，只用于匹配硬件的公开验证：
...
- 默认权重：官方旗舰 B11 Transformer `kata1-tf3-b11c768-s12002M-d6304M.bin.gz`，界面显示为“Transformer B11 · 2026-09-25”
- 默认权重大小：`262,017,809` 字节（约 250 MiB），SHA-256：`4a6312e80faadee7b7dd28689a2e87a1efb4640c10132f16290da7a17b4c6d9e`
...
- `智子云算力`：手机号/邮箱登录后，软件会创建一个虚拟 KataGo 引擎，主引擎、实时分析、AI 对局、引擎对局、Kata形势和闪电分析都走同一类 GTP 桥接体验。
...
- `自建算力`：可直接粘贴兼容 KaTrain 的 `ws://` / `wss://` 分析链接，或导入二维码。还没有链接时，可在 Linux x86_64 NVIDIA GPU 服务器使用 [KataGo 远程算力一键部署](https://github.com/wimi321/katago-remote-one-click)，运行一条命令生成加密 `WSS` 链接和二维码，无需自行开放公网端口。
- `旧版 SSH 兼容`：已有 SSH 远程引擎仍可通过引擎设置继续使用，不会被自建算力配置覆盖。
...
- Windows 64 位提供 `opencl`、`with-katago`、统一 `nvidia` 三条内置引擎线的安装器和便携包
