...
第一版不做文件选择器，避免引入额外 UI 分支。默认输出目录为应用目录下的诊断目录，优先解析当前 `lizzieyzy-next` 工作目录或构建目录：

```text
<lizzieyzy-next>/sync-diagnostics/
```
...
测试可以注入临时输出目录；UI 默认不写 `target/` 或用户主目录。若运行时无法从 `user.dir` 或 code source 定位 `lizzieyzy-next`，才退回当前工作目录下的 `sync-diagnostics/`。
