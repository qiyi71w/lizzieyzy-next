 ...
 * another press starts, and it stops on release, pointer leave, focus loss, disable, or any control
 * hierarchy/displayability change.
 */
final class KomiHoldSession {
  ...
  static KomiHoldSession attach(
    ...
    KomiHoldSession session =
        new KomiHoldSession(control, holdStep, initialDelayMs, repeatDelayMs, null);
    session.bind();
    return session;
  }

  static KomiHoldSession attachForTesting(
      AbstractButton control,
      Runnable holdStep,
      int initialDelayMs,
      int repeatDelayMs,
      BooleanSupplier lifecycleReady) {
    KomiHoldSession session =
        new KomiHoldSession(
            control, holdStep, initialDelayMs, repeatDelayMs, lifecycleReady);
    session.bind();
    return session;
  }
  ...
  boolean isFocusManagerBound() {
    return focusManagerBound;
  }

  private void bind() {
    control.addMouseListener(mouseAdapter);
    control.addFocusListener(focusAdapter);
    control.addComponentListener(componentAdapter);
  ...
  private void onHierarchyChanged(HierarchyEvent event) {
    if (!SwingUtilities.isEventDispatchThread()) {
      SwingUtilities.invokeLater(() -> onHierarchyChanged(event));
      return;
    }
  ...
  private boolean canRepeat() {
    return control.isEnabled() && lifecycleReady.getAsBoolean();
  }

  private boolean hasActiveWindow() {
    Window window = SwingUtilities.getWindowAncestor(control);
    return control.isShowing() && window != null && window.isEnabled();
  }
