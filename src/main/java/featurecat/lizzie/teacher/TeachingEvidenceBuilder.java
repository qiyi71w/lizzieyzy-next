...
public final class TeachingEvidenceBuilder {
  ...
  public enum TeachingConfidence {
    high,
    medium,
    low
  }
  ...
  public static class TeachingEvidenceCandidate {
    public String move;
    public Double winrate, scoreLead;
    public String perspectiveColor;
    public Double blackWinrate, blackScoreLead;
  ...
  public static class PlayedMoveView {
    public String move;
    public Double winrate, scoreLead;
    public String perspectiveColor;
    public Double blackWinrate, blackScoreLead;
  ...
  public static class LossView {
    public Double winrateLoss, scoreLoss;
    public TeachingSeverity severity;
    public TeachingConfidence confidence;
  ...
  public static class RecognizedMotifView {
    public String id,
        ...
        josekiFamily;
    public Double score;
    public List<String> relatedMoves = new ArrayList<>();
    public List<String> sourceRefs = new ArrayList<>();
    public List<Object> expectedNextMoves = new ArrayList<>();
    public int variationCount;
  }

  public static class KnowledgeReference {
    public String id, title, confidence, whyMatched, matchType;
    public Double score;
    public List<String> keyVariations = new ArrayList<>();
  }

  public static class VariationHint {
    public String move, purpose, expectedReply, result, confidence;
    public List<String> pv = new ArrayList<>();
  }

  public static class StudentView {
    public String id, level;
    public List<String> recurringIssues = new ArrayList<>();
  }
  ...
  public static class MarkdownVerification {
    ...
    public List<String> allowedMoves = new ArrayList<>();
  }
  ...
  static String roundStr(Double v, int digits) {
    return String.format("%." + digits + "f", round(v, digits));
  }

  static TeachingPhase inferPhase(int moveNumber) {
    return moveNumber <= 50
        ? TeachingPhase.opening
        : moveNumber <= 160 ? TeachingPhase.middle : TeachingPhase.endgame;
  }
  ...
  static TeachingSeverity inferSeverity(double winrateLoss, double scoreLoss, String judgement) {
    ...
    if (judgement != null && judgement.equals("blunder") || winrateLoss >= 15 || scoreLoss >= 8)
      return TeachingSeverity.blunder;
    if (judgement != null && judgement.equals("mistake") || winrateLoss >= 7 || scoreLoss >= 4)
      return TeachingSeverity.mistake;
    if (judgement != null && judgement.equals("inaccuracy")
        || winrateLoss >= 2.5
        || scoreLoss >= 1.5) return TeachingSeverity.inaccuracy;
  ...
  static long totalVisits(List<TeachingEvidenceCandidate> cands) {
    long t = 0;
    if (cands != null) for (var c : cands) t += c.visits;
    return t;
  }

  static class Conf {
    TeachingConfidence confidence;
    String reason;
    TeachingMode mode;
  }
  ...
  static long candidateTotalVisits(AnalysisBrain.PvReport pv) {
    long t = 0;
    if (pv != null && pv.candidates != null)
      for (AnalysisBrain.PvCandidate c : pv.candidates) t += c.visits;
    return t;
  }
  ...
  static Conf inferConfidence(
      AnalysisBrain.PvReport pv,
      TeachingSeverity severity,
      double winrateLoss,
      double scoreLoss,
      String judgement,
      double bestVisits,
      long playedVisits,
      boolean hasForcedPlayed) {
    Conf c = new Conf();
    ...
    if (visits < 80 || (playedVisits == 0 && hasForcedPlayed)) {
      ...
      c.reason =
          "KataGo visits are low ("
              + visits
              + "); treat the explanation as a teaching hypothesis, not a final verdict.";
      c.mode = TeachingMode.needs_deeper_search;
      return c;
    }
    if (severity == TeachingSeverity.good && spread < 1.5) {
      ...
      c.reason =
          "Top candidates are close ("
              + round(spread, 1)
              + " winrate points apart); explain as a choice of style/direction.";
      c.mode = TeachingMode.style_choice;
      return c;
    }
    if (severity == TeachingSeverity.inaccuracy && spread < 2) {
      ...
      c.mode = TeachingMode.candidate_choice;
      return c;
    }
    if ((severity == TeachingSeverity.mistake || severity == TeachingSeverity.blunder)
        && visits >= 250
        && consistentLoss) {
      c.confidence = TeachingConfidence.high;
      c.reason = "Winrate and score loss agree, with enough visits (" + visits + ").";
      c.mode = TeachingMode.clear_mistake;
      return c;
    }
    c.confidence = visits >= 160 ? TeachingConfidence.medium : TeachingConfidence.low;
    c.reason =
        "Evidence is usable but not decisive: visits="
            + visits
            + ", spread="
            + round(spread, 1)
            + ", winrateLoss="
            ...
            + ".";
    c.mode =
        severity == TeachingSeverity.good
            ? TeachingMode.style_choice
            : TeachingMode.candidate_choice;
    return c;
  }
  ...
  static boolean strongMatch(KnowledgeReference m) {
    return m != null && ("strong".equals(m.confidence) || "exact".equals(m.confidence));
  }
  ...
  static RecognizedMotifView recognizedJoseki(List<RecognizedMotifView> motifs) {
    ...
    for (RecognizedMotifView m : motifs)
      if (m.motifType != null
          && m.motifType.startsWith("joseki:")
          && ("strong".equals(m.confidence) || "medium".equals(m.confidence))) return m;
    return null;
  }

  static String focusFromEvidence(
      TeachingPhase phase,
      double winrateLoss,
      List<KnowledgeReference> matches,
      List<RecognizedMotifView> motifs) {
    ...
    if (phase == TeachingPhase.opening
        && (strongMatch(joseki) || recognizedJoseki(motifs) != null)) {
  ...
  static String candidateConfidence(long visits) {
    return visits >= 250 ? "high" : visits >= 80 ? "medium" : "low";
  }
  ...
  static long playedVisitsOf(MoveAnalysis analysis) {
    ...
    for (AnalysisBrain.PvCandidate c : analysis.pv.candidates)
      if (c.move != null && c.move.equals(analysis.actualMove)) return c.visits;
    return 0;
  }
  ...
  static List<TeachingEvidenceCandidate> uniqueHintCandidates(MoveAnalysis analysis) {
    ...
    if (analysis != null && analysis.actualMove != null) {
      for (TeachingEvidenceCandidate c : candidates)
        if (c.move != null && c.move.equals(analysis.actualMove)) {
          played = c;
          break;
        }
    }
    ...
    for (TeachingEvidenceCandidate c : new TeachingEvidenceCandidate[] {first, played, second}) {
      if (c == null) continue;
      boolean dup = false;
      for (TeachingEvidenceCandidate e : selected)
        if (e.move != null && e.move.equals(c.move)) {
          dup = true;
          break;
        }
  ...
  static TeachingPacingAdvice buildTeachingPacingAdvice(
      MoveAnalysis analysis,
      List<KnowledgeReference> knowledgeMatches,
      List<RecognizedMotifView> recognizedMotifs) {
    TeachingPacingAdvice advice = new TeachingPacingAdvice();
    TeachingPhase phase = inferPhase(analysis != null ? analysis.moveNumber : 0);
    double winrateLoss =
        analysis != null && analysis.classification != null
            ? analysis.classification.winrateLoss
            : 0;
    double scoreLoss =
        analysis != null && analysis.classification != null ? analysis.classification.scoreLoss : 0;
    TeachingSeverity severity =
        inferSeverity(
            winrateLoss,
            scoreLoss,
            analysis != null && analysis.classification != null
                ? analysis.classification.severity.name().toLowerCase()
                : "unknown");
    Conf confidence =
        inferConfidence(
            analysis != null ? analysis.pv : null,
            severity,
            winrateLoss,
            scoreLoss,
            analysis != null && analysis.classification != null
                ? analysis.classification.severity.name().toLowerCase()
                : "",
            analysis != null && analysis.best != null ? analysis.best.visits : 0,
            playedVisitsOf(analysis),
            false);
    ...
    boolean tactical =
        strongMatch(matchByType(knowledgeMatches, "life_death"))
            || strongMatch(matchByType(knowledgeMatches, "tesuji"));
    boolean hasJosekiBranch =
        joseki != null
            && ("partial".equals(joseki.confidence)
                || (joseki.keyVariations != null && !joseki.keyVariations.isEmpty())
                || winrateLoss >= 2);
    ...
    if (confidence.confidence == TeachingConfidence.low) {
      ...
      advice.whyThisMuchExplanation = "KataGo 搜索或实战手证据还不够强，只能讲判断倾向，不能下绝对结论。";
        ...
        || phase == TeachingPhase.middle
        || severity == TeachingSeverity.mistake
        || severity == TeachingSeverity.blunder
        || winrateLoss >= 7
        || scoreLoss >= 4) {
      advice.teachingDensity = TeachingDensity.detailed;
      advice.whyThisMuchExplanation = "这是中盘战、急所计算或明显损失局面，需要讲清这手目的、对方应手、PV 后续和实战代价。";
    } else if (hasJosekiBranch
        || "joseki-branch".equals(focus)
        || (phase == TeachingPhase.opening && winrateLoss >= 2)) {
      advice.teachingDensity = TeachingDensity.branch;
      advice.whyThisMuchExplanation = "这是定式分支、布局选择或相似型局面，适合列 1-2 个关键变化和选择条件。";
    } else if (("joseki-normal".equals(focus) && winrateLoss < 2)
        || (severity == TeachingSeverity.good && winrateLoss < 2)) {
    ...
    for (TeachingEvidenceCandidate candidate : uniqueHintCandidates(analysis)) {
      ...
      h.purpose =
          hintPurpose(
              ...
              focus,
              analysis.actualMove != null && candidate.move.equals(analysis.actualMove));
      String expectedReply = null;
      for (String pvMove : candidate.pv)
        if (pvMove != null && !pvMove.equals(candidate.move)) {
          expectedReply = pvMove;
          break;
        }
      h.expectedReply = expectedReply;
      h.pv = candidate.pv.size() > 6 ? candidate.pv.subList(0, 6) : candidate.pv;
      String summaryText =
          candidate.scoreSummary != null && !candidate.scoreSummary.isEmpty()
              ? candidate.scoreSummary
              : "目差 " + roundStr(candidate.scoreLead, 1);
      h.result =
          "胜率 "
              + roundStr(candidate.winrate, 1)
              + "%，"
              + summaryText
              + "，搜索 "
              + candidate.visits
  ...
  public static TeachingEvidence buildTeachingEvidence(
      MoveAnalysis analysis,
      String userPrompt,
      List<MotifRecognizer.RecognizedTeachingMotif> motifs,
      List<KnowledgeReference> knowledgeMatches,
      List<Object> recommendedProblems) {
    ...
    TeachingPacingAdvice pacing =
        buildTeachingPacingAdvice(analysis, new ArrayList<>(), motifViews);
    ...
    if (analysis != null) {
      ...
      ev.before.scoreLead =
          round(ScorePerspective.scoreLeadForColor(analysis.beforeScoreLead, true), 2);
      ...
      ev.before.scoreSummary =
          featurecat.lizzie.teacher.analysis.ScorePerspective.scoreSummaryFromBlackLead(
                  analysis.beforeScoreLead, perspectiveColor)
              .text;
      ev.afterActual.winrate = round(analysis.afterWinrate, 2);
      ev.afterActual.scoreLead =
          round(ScorePerspective.scoreLeadForColor(analysis.afterScoreLead, true), 2);
      ...
      ev.afterActual.scoreSummary =
          featurecat.lizzie.teacher.analysis.ScorePerspective.scoreSummaryFromBlackLead(
                  analysis.afterScoreLead, perspectiveColor)
              .text;
      ...
      ev.playedMove.scoreLead =
          round(ScorePerspective.scoreLeadForColor(analysis.actualScoreLead, true), 2);
      ...
      ev.playedMove.scoreSummary =
          featurecat.lizzie.teacher.analysis.ScorePerspective.scoreSummaryFromBlackLead(
                  analysis.actualScoreLead, perspectiveColor)
              .text;
      ...
      for (AnalysisBrain.PvCandidate c : analysis.pv.candidates) {
        ...
        tc.scoreSummary =
            featurecat.lizzie.teacher.analysis.ScorePerspective.scoreSummaryFromBlackLead(
                    c.scoreLead != null ? c.scoreLead : analysis.actualScoreLead, perspectiveColor)
                .text;
        tc.visits = c.visits;
        tc.order = c.rank;
        tc.rank = c.rank;
    ...
    ev.loss.winrateLoss = winrateLoss;
    ev.loss.scoreLoss = scoreLoss;
    ev.loss.severity = severity;
    ev.loss.confidence = confidence.confidence;
    ev.loss.confidenceReason = confidence.reason;
    ev.loss.teachingMode = confidence.mode;
    ev.teachingDensity = pacing.teachingDensity;
    ev.teachingFocus = pacing.teachingFocus;
    ev.whyThisMuchExplanation = pacing.whyThisMuchExplanation;
    ...
    for (KnowledgeReference kr :
        knowledgeMatches.subList(0, Math.min(6, knowledgeMatches.size()))) {
  ...
  static List<RecognizedMotifView> toMotifViews(
      List<MotifRecognizer.RecognizedTeachingMotif> motifs) {
    ...
    for (MotifRecognizer.RecognizedTeachingMotif m : motifs.stream().limit(8).toList()) {
      ...
      v.id = m.id;
      v.title = m.title;
      v.motifType = m.motifType;
      v.confidence = m.confidence.name().toLowerCase();
      v.score = m.score;
      v.whyMatched = m.whyMatched;
      v.recognition = m.recognition;
      v.wrongThinking = m.wrongThinking;
      v.correctThinking = m.correctThinking;
      v.drillPrompt = m.drillPrompt;
      v.sourceQuality = m.sourceQuality;
      v.josekiFamily = m.josekiFamily;
      v.relatedMoves =
          m.relatedMoves != null
              ? new ArrayList<>(java.util.Arrays.asList(m.relatedMoves))
              : new ArrayList<>();
      v.sourceRefs =
          m.sourceRefs != null
              ? new ArrayList<>(java.util.Arrays.asList(m.sourceRefs))
              : new ArrayList<>();
  ...
  static List<String> allowedMoves(TeachingEvidence evidence) {
    ...
    for (String m : moves)
      if (m != null && !m.toLowerCase().equals("pass")) out.add(m.toUpperCase());
    return out;
  }
  ...
  static boolean nearRecommendationPhrase(String markdown, String coord) {
    ...
    String window =
        markdown.substring(
            Math.max(0, index - 24), Math.min(markdown.length(), index + coord.length() + 24));
    return window.matches(".*(推荐|最佳|首选|应该|建议|好点|更好|best|recommend|should|play\\s+at|候補|추천|권장).*");
  }

  public static MarkdownVerification verifyTeacherMarkdown(
      String markdown, TeachingEvidence evidence) {
    ...
    if (markdown != null) {
      for (String coord : extractCoordinates(markdown)) {
        if (!allowedSet.contains(coord) && nearRecommendationPhrase(markdown, coord)) {
          violations.add(
              "Unsupported recommended coordinate "
                  + coord
                  + "; it is not in top candidates, actual move, or PV evidence.");
        }
      }
      ...
      while (pm.find()) {
        try {
          percents.add(Double.parseDouble(pm.group(1)));
        } catch (Exception ignore) {
        }
      }
      for (Double value : percents)
        if (value > 100) violations.add("Impossible winrate percentage " + value + "%.");
      if (percents.size() >= 5)
        warnings.add(
            "Many percentages were mentioned; consider simplifying the teacher explanation.");
      if (evidence.loss.confidence != TeachingConfidence.high
          && markdown.matches(".*(明显恶手|必败|唯一|绝对|certainly|only\\s+move|forced).*"))
      ...
      for (RecognizedMotifView m : evidence.recognizedMotifs)
        if (m.motifType != null
            && m.motifType.startsWith("joseki:")
            && ("strong".equals(m.confidence) || "medium".equals(m.confidence)))
          supportedJoseki = true;
      if (mentionsJoseki && !supportedJoseki)
        warnings.add(
            "Joseki terminology was used without a medium/strong recognized joseki motif.");
      if (markdown.matches(".*(据.*(Sensei|Kogo|GoGoD|Wikibooks)|source says|according to).*"))
        warnings.add(
            "Teacher appears to cite external sources; sourceRefs are traceability labels, not source text.");
    }
  ...
  static String evidenceSummaryZh(TeachingEvidence evidence) {
    ...
    return "AI 证据链：第 "
        ...
        + best
        + "，胜率损失 "
        + round(evidence.loss.winrateLoss, 1)
        + "%，目差损失约 "
        + round(evidence.loss.scoreLoss, 1)
        + "，判断 "
        + evidence.loss.severity
        + "，置信度 "
        + evidence.loss.confidence
        + "。";
  }
  ...
  static String evidenceSummaryEn(TeachingEvidence evidence) {
    String best =
        evidence.bestCandidates.isEmpty() ? "unknown" : evidence.bestCandidates.get(0).move;
    String actual = evidence.actualMove != null ? evidence.actualMove : "unknown";
    return "Evidence chain: move "
        + evidence.moveNumber
        + ", played "
        + actual
        + ", top candidate "
  ...
  public static String buildVerificationNote(
      MarkdownVerification verification, TeachingEvidence evidence, String localeInput) {
    String locale =
        localeInput != null
                && (localeInput.equals("zh-TW")
                    || localeInput.equals("en-US")
                    || localeInput.equals("ja-JP")
                    || localeInput.equals("ko-KR")
                    || localeInput.equals("th-TH")
                    || localeInput.equals("vi-VN"))
            ? localeInput
            : "zh-CN";
    String summary =
        locale.equals("zh-CN")
            ? evidenceSummaryZh(evidence)
            : locale.equals("zh-TW") ? evidenceSummaryTw(evidence) : evidenceSummaryEn(evidence);
    String motifLine = "";
      ...
      if (locale.equals("zh-TW")) {
        motifLine =
            "\n> 識別棋形："
                ...
                + "，score "
                + round(motif.score, 1)
                + (motif.sourceRefs != null && !motif.sourceRefs.isEmpty()
                    ? "，來源標記 " + String.join("/", motif.sourceRefs)
                    : "")
                + "）。";
        ...
        motifLine =
            "\n> 识别棋形："
                ...
                + round(motif.score, 1)
                + (motif.sourceRefs != null && !motif.sourceRefs.isEmpty()
                    ? "，来源标记 " + String.join("/", motif.sourceRefs)
                    : "")
                + "）。";
      } else {
        motifLine =
            "\n> Recognized motif: "
                ...
                + round(motif.score, 1)
                + (motif.sourceRefs != null && !motif.sourceRefs.isEmpty()
                    ? ", sources " + String.join("/", motif.sourceRefs)
                    : "")
                + ").";
      }
    }
    ...
    if (verification != null) {
      issueLines.addAll(verification.violations);
      issueLines.addAll(verification.warnings);
    }
    String issues = "";
    if (!issueLines.isEmpty()) {
      List<String> shown = issueLines.size() > 3 ? issueLines.subList(0, 3) : issueLines;
      String prefix =
          locale.equals("zh-CN") ? "校验提示" : locale.equals("zh-TW") ? "校驗提示" : "Verifier notes";
  ...
  public static String appendVerificationNote(
      String markdown,
