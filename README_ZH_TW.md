...
點擊頂端的搜尋按鈕，或按 `Ctrl+K`（macOS 為 `Command+K`）。可用任一支援語言輸入功能名稱，搜尋完全離線；輸入為空時按向下鍵或選擇「瀏覽功能」，即可按分類查看完整目錄。用方向鍵選擇結果，輸入法候選送出後要再單獨按一次 `Enter` 才會啟用。選單和對話框結果會開啟原有入口；設定結果只會定位並聚焦控制項，不會自動修改值。暫時無法使用的結果仍會顯示原因，按 `Esc` 可關閉搜尋並回到原來的焦點。
