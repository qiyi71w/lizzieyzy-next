...
点击顶部的搜索按钮、选择“设置 → 搜索功能”，或按 `Ctrl+K`（macOS 为 `Command+K`）。可用任一支持语言输入功能名，检索完全离线；输入为空时按向下键或选择“浏览功能”即可按分类查看完整目录。用上下键选择结果，输入法候选提交后再单独按一次 `Enter` 才会激活。菜单和对话框功能会打开原有入口；设置结果只负责定位并聚焦控件，不会自动修改值。暂不可用的结果仍会显示原因，按 `Esc` 可关闭搜索并回到原来的焦点。
