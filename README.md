...
> [!IMPORTANT]
> 先知道这 5 件事就够了：
> - Windows 大多数用户下载 `windows64.opencl.installer.exe`，这是 **OpenCL 版（推荐）**
> - 如果 OpenCL 在你的电脑上表现不好，再改用 `windows64.with-katago.installer.exe`
...
- 大多数用户先下 **`windows64.opencl.installer.exe`**，这是速度优先的 **OpenCL 版**
- 如果 OpenCL 在你的电脑上不稳定，再下 **`windows64.with-katago.installer.exe`**，这是兼容兜底的 **CPU 版**
- 如果你的电脑有 **NVIDIA 显卡**，并且你更在意分析速度，先下 **`windows64.nvidia.installer.exe`**

前两个分别是 OpenCL 推荐版和 CPU 兼容版，最后一个是英伟达显卡专用的极速整合包。
...
| 你的电脑 | 直接下载这个 |
| --- | --- |
| Windows 64 位，OpenCL 版，推荐更快 | `windows64.opencl.installer.exe` |
| Windows 64 位，OpenCL 版，免安装 | `windows64.opencl.portable.zip` |
| Windows 64 位，CPU 版，兼容兜底 | `windows64.with-katago.installer.exe` |
...
- Windows：不知道自己该选哪个，就先下 `windows64.opencl.installer.exe`
- Mac：先看自己是 Apple Silicon 还是 Intel
- Linux：直接下 `with-katago.zip`

一眼记住版本定位：

- `opencl.installer.exe`：OpenCL 版，Windows 推荐首选
- `with-katago.installer.exe`：CPU 版，适合 OpenCL 表现不好的机器
- `nvidia.installer.exe`：只给 NVIDIA 显卡用户，默认分析速度更高
- `opencl.portable.zip`：OpenCL 版免安装
- `nvidia.portable.zip`：给 NVIDIA 显卡用户的免安装版
- `with-katago.portable.zip`：CPU 版免安装，兼容兜底
