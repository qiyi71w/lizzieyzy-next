...
def inspect_pe(headers: str, dependencies: str, bundled: set[str] | None = None,
               extra_system: set[str] | None = None) -> dict:
    ...
    allowed = SYSTEM_LIBRARIES | {name.lower() for name in (bundled if bundled is not None else {"opencl.dll"})} | (extra_system or set())
    for name in needed:
        lowered = name.lower()
        if lowered not in allowed and not lowered.startswith("api-ms-win-"):
            raise ValueError(f"Unapproved PE dependency: {name}")
    return {"needed": needed}


def verify_runtime_closure(audits: dict, bundled: set[str], present: set[str]) -> None:
    bundled = {name.lower() for name in bundled}
    present = {name.lower() for name in present}
    if not bundled.issubset(present):
        raise ValueError("Required runtime missing from portable bundle")
    for audit in audits.values():
        for name in audit["needed"]:
            if name.lower() in bundled and name.lower() not in present:
                raise ValueError(f"Unresolved portable dependency: {name}")
...
def package(build: Path, sdk: Path, output: Path) -> dict:
    ...
    try:
        ...
        verify_runtime_closure(audits, bundled, {path.name for path in output.iterdir() if path.is_file()})
