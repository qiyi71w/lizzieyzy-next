...
상단 검색 버튼을 누르거나 **설정 → 기능 검색**을 선택하거나 `Ctrl+K`(macOS에서는 `Command+K`)를 누르세요. 지원되는 어떤 언어의 기능 이름으로도 오프라인 검색할 수 있습니다. 입력이 비어 있을 때 아래쪽 화살표를 누르거나 **기능 둘러보기**를 선택하면 전체 목록을 범주별로 볼 수 있습니다. 화살표 키로 결과를 선택합니다. IME 후보를 확정한 뒤 별도의 동작으로 `Enter`를 한 번 더 눌러야 실행됩니다. 메뉴와 대화 상자 결과는 기존 진입점을 열고, 설정 결과는 해당 컨트롤을 찾아 포커스할 뿐 값을 자동으로 바꾸지 않습니다. 현재 사용할 수 없는 결과도 이유와 함께 표시되며, `Esc`를 누르면 검색을 닫고 이전 포커스로 돌아갑니다.
