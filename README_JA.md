...
上部バーの検索ボタンをクリックするか、`Ctrl+K`（macOS では `Command+K`）を押します。対応しているどの言語の機能名でもオフラインで検索できます。入力が空のときは下矢印キーを押すか **機能一覧** を選ぶと、全項目をカテゴリ別に表示します。矢印キーで結果を選択します。IME の候補を確定したあと、別の操作としてもう一度 `Enter` を押すと実行されます。メニューやダイアログの結果は既存の入口を開き、設定の結果は対象を表示してフォーカスするだけで、値を自動変更しません。現在使えない結果にも理由が表示され、`Esc` で検索を閉じると元のフォーカスに戻ります。
